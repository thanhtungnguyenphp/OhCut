...
class TestJobCreation:
    ...
    def test_create_job(self, temp_db):
        ...
        job_id = temp_db.create_job(
            job_type="cut", input_files=["movie.mp4"], config={"duration": 60, "copy_codec": True}
        )
    ...
    def test_create_job_with_profile(self, temp_db):
        ...
        job_id = temp_db.create_job(
            ...
            config={"duration": 11, "copy_codec": False, "profile_name": "clip_720p"},
        )
    ...
    def test_job_initial_status(self, temp_db):
        ...
        job_id = temp_db.create_job(job_type="cut", input_files=["test.mp4"], config={})
...
class TestJobRetrieval:
    ...
    def test_get_job_by_id(self, temp_db):
        ...
        job_id = temp_db.create_job(
            job_type="cut", input_files=["movie.mp4"], config={"duration": 60}
        )
    ...
    def test_job_attributes(self, temp_db):
        ...
        job_id = temp_db.create_job(
            job_type="concat", input_files=["a.mp4", "b.mp4"], config={"test": "value"}
        )
...
class TestJobStatusUpdates:
    ...
    def test_update_status_to_completed(self, temp_db):
        ...
        temp_db.update_job_status(
            job_id, JobStatus.COMPLETED, progress=100.0, output_files=["out1.mp4", "out2.mp4"]
        )
    ...
    def test_update_status_to_failed(self, temp_db):
        ...
        temp_db.update_job_status(job_id, JobStatus.RUNNING)
        temp_db.update_job_status(job_id, JobStatus.FAILED, error_message="FFmpeg command failed")
...
class TestJobQueries:
    ...
    def test_list_all_jobs(self, temp_db):
        ...
        for i in range(5):
            temp_db.create_job(job_type="cut", input_files=[f"video{i}.mp4"], config={})
...
class TestJobCleanup:
    ...
    def test_cleanup_old_completed_jobs(self, temp_db):
        ...
        cursor.execute("UPDATE jobs SET completed_at = ? WHERE id = ?", (old_date, job_id))
        conn.commit()
        conn.close()
    ...
    def test_cleanup_only_completed_jobs(self, temp_db):
        ...
        old_date = (datetime.now() - timedelta(days=60)).isoformat()
        cursor.execute(
            "UPDATE jobs SET completed_at = ? WHERE id IN (?, ?)", (old_date, job1, job2)
        )
        conn.commit()
        conn.close()
...
class TestEdgeCases:
    ...
    def test_empty_input_files(self, temp_db):
        ...
        job_id = temp_db.create_job(job_type="cut", input_files=[], config={})
    ...
    def test_long_error_message(self, temp_db):
        ...
        temp_db.update_job_status(job_id, JobStatus.FAILED, error_message=long_error)
